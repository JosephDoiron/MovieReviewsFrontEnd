...
export default function FeaturedMovieCard({ featured }: FeaturedMovieCardProps) {
  ...
  return (
    <div className="bg-navy rounded-xl overflow-hidden shadow-lg flex flex-col md:flex-row">
      {/* Poster */}
      <div className="md:w-48 md:flex-shrink-0 bg-navy">
        <img
      ...
      <div className="p-5 flex flex-col justify-between flex-1">
        <div>
          ...
          <h3 className="text-white font-semibold text-xl mb-2">
            <Link to={`/movies/${movie.id}`} className="hover:text-gold transition-colors">
              {movie.title}
            </Link>
          </h3>
        ...
        {latestReview && (
          <div className="border-t border-white/10 pt-3 mt-auto">
            ...
            <p className="text-teal text-xs mt-1 font-medium">
              &mdash;{" "}
              {latestReview.authorId ? (
                <Link to={`/critic/${latestReview.authorId}`} className="hover:underline">
                  {latestReview.criticName}
                </Link>
              ) : (
                latestReview.criticName ?? "Anonymous"
              )}
            </p>
          </div>
        )}
